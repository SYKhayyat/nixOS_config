...
let
  ...
  # ── The ~/.emacs.d bridge, and why it is not optional ───────────────────
  #
  # Emacs 30 picks `~/.emacs.d' as `user-emacs-directory' when that directory
  # exists, and on this machine it does: it predates the config moving to
  # ~/.config/emacs and still holds the user's snippets, undo-tree history, a
  # MELPA `elpa/' and the state files Emacs writes there. Removing it is a data
  # migration, not a cleanup, so the config stays reachable the way the flat
  # tree arranged before the consolidation: two store-backed bridges in
  # ~/.emacs.d that point `user-emacs-directory' at ~/.config/emacs and load on
  # from there.
  #
  # This is restored rather than newly invented -- commit 5dc15df ("Restore
  # declarative Emacs bridge") put it in emacs.nix and the consolidation did
  # not carry it over. What that leaves is worse than a missing feature: the
  # bridges installed by the *previous* generation are still on disk, pointing
  # at store paths nothing in this generation references. They work today and
  # dangle after the first `nix-collect-garbage' -- after which `emacs' opens
  # with no configuration at all, silently, which is the exact shape of
  # failure the rest of this repo is written against.
  emacsInitBridge = pkgs.writeText "emacs-init-bridge.el" ''
    ...
    ;; Generated by modules/home/emacs/default.nix in nixos-config. The real
    ;; config lives in ~/.config/emacs, which home-manager owns; this file
    ;; exists only because Emacs prefers ~/.emacs.d when it is there.
...
in
{
  ...
  # ── Put the bridges in place, without root ──────────────────────────────
  #
  # `home.activation` rather than the `systemd.tmpfiles` rules the flat tree
  # used: these are files in the user's home, and this repo's rule is that the
  # home layer owns those. It also needs no privilege -- the two symlinks there
  # today are root-owned, but they sit in a user-owned directory, so `rm -f'
  # from here can replace them.
  #
  # Idempotent, and it repairs rather than only creates: a bridge that is
  # missing, has been replaced by a real file, or still points at the store
  # path of an older generation is re-linked. `readlink' is compared, not the
  # file's existence, because the failure being fixed is a symlink that resolves
  # to nothing -- which `-e' cannot see either.
  home.activation.emacsDotDBridge = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    emacsDotD="${homeDir}/.emacs.d"
    if [ -d "$emacsDotD" ]; then
      if [ ! -L "$emacsDotD/init.el" ] || [ "$(readlink "$emacsDotD/init.el")" != "${emacsInitBridge}" ]; then
        $DRY_RUN_CMD rm -f "$emacsDotD/init.el"
        $DRY_RUN_CMD ln -s "${emacsInitBridge}" "$emacsDotD/init.el"
      fi
      if [ ! -L "$emacsDotD/early-init.el" ] || [ "$(readlink "$emacsDotD/early-init.el")" != "${emacsEarlyInitBridge}" ]; then
        $DRY_RUN_CMD rm -f "$emacsDotD/early-init.el"
        $DRY_RUN_CMD ln -s "${emacsEarlyInitBridge}" "$emacsDotD/early-init.el"
      fi
    fi
  '';
